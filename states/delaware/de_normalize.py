...
def run_enrichment(df: pd.DataFrame, skip_enrichment: bool = False) -> pd.DataFrame:
    """Run crash_enricher Tier 1 + Road Inventory enrichment."""
    if skip_enrichment:
    ...
    try:
        ri_path = _CACHE_DIR / f"{STATE_ABBREVIATION}_road_inventory.parquet.gz"
        ri_ready = ri_path.exists()
        print(f"  [8/8] Phase 8: Enrichment (Tier 1 + Road Inventory {'✅' if ri_ready else '⚠️  NOT FOUND'})...")
        if not ri_ready:
            print(f"         Road inventory not at: {ri_path}")
            print(f"         Build with: python build_road_inventory.py --state {STATE_ABBREVIATION}")
        ...
        # Always run — enricher handles road inventory detection internally
        df = enricher.enrich_all(df, skip_tier2=False)
        return df
