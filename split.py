#!/usr/bin/env python3
"""
split.py — CrashLens Universal Jurisdiction & Road Type Splitter  v3.0
=======================================================================
...
parquet.gz files that exactly match the Cloudflare R2 folder structure the
...
def write_road_type_splits(
    ...
    """Apply each road type filter and write parquet files. Returns {type: row_count}."""
    counts = {}
    for road_type, spec in road_types.items():
        filtered = spec["filter"](df) if spec["filter"] is not None else df
        counts[road_type] = write_file(
            filtered,
            output_dir / f"{road_type}.parquet",
            dry_run,
        )
    return counts
...
def split_state_level(
    ...
    counts = write_road_type_splits(df, state_dir, ROAD_TYPES_STATE_REGION, dry_run)
    return counts
    ...
    if strategy == "column":
        ...
        for region_name, region_df in groups:
            ...
            all_counts[region_key] = write_road_type_splits(
                ...
                ROAD_TYPES_STATE_REGION, dry_run
            )
    else:
...
def cleanup_legacy_r2(
    ...
    """
    Remove legacy duplicate files from R2 after successful split + upload.
    ...
    try:
        # List ALL files recursively under state prefix
        result = subprocess.run(
            ["aws", "s3", "ls", f"s3://{r2_bucket}/{state_prefix}/",
             "--endpoint-url", r2_endpoint, "--recursive"],
            capture_output=True, text=True, timeout=60)
        if result.returncode != 0:
            print("    Could not list R2 — skipping cleanup")
            return 0

        # Parse: {date} {time} {size} {key}
        r2_files = {}  # full_key → size
        ...
        print("    aws CLI not available — skipping cleanup")
        return 0

    # ── Check 1: statewide_all_roads duplicate in _state/ ──
    statewide_key = f"{state_prefix}/_state/statewide_all_roads.csv.gz"
    all_roads_key = None
    all_roads_size = 0
    ...
    for candidate in [f"{state_prefix}/_state/all_roads.parquet",
        ...
        if candidate in r2_files:
            ...
            all_roads_size = r2_files[candidate]
            break

    if statewide_key in r2_files:
        statewide_size = r2_files[statewide_key]

        if all_roads_key is None:
            print(f"    ⚠️ statewide_all_roads.csv.gz exists but no all_roads found — keeping")
        else:
            # Same format → check size match; different format → all_roads exists is enough
            same_format = all_roads_key.endswith(".csv.gz")
            safe = True
            if same_format:
                ratio = statewide_size / max(all_roads_size, 1)
                safe = 0.95 <= ratio <= 1.05

            if safe:
                size_mb = statewide_size / 1024 / 1024
                if dry_run:
                    print(f"    [DRY] Would delete statewide_all_roads.csv.gz ({size_mb:.1f} MB)")
                    removed += 1
                else:
                    cmd = ["aws", "s3", "rm", f"s3://{r2_bucket}/{statewide_key}",
                           "--endpoint-url", r2_endpoint, "--only-show-errors"]
                    res = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
                    if res.returncode == 0:
                        print(f"    ✅ Deleted statewide_all_roads.csv.gz ({size_mb:.1f} MB)")
                        removed += 1
            else:
    ...
    for csv_key in csv_keys:
        ...
        size_mb = r2_files[csv_key] / 1024 / 1024
    ...
    if all_roads_key and all_roads_key.endswith(".parquet"):
        ...
        for key, size in list(r2_files.items()):
            is_statewide_csv = False
            for pattern in statewide_patterns:
                if key.startswith(pattern) and (key.endswith(".csv") or key.endswith(".csv.gz")):
                    is_statewide_csv = True
                    break

            if not is_statewide_csv:
                continue

            size_mb = size / 1024 / 1024
            short_name = key.replace(f"{state_prefix}/", "")

            if dry_run:
                print(f"    [DRY] Would delete {short_name} ({size_mb:.0f} MB) — split complete, parquet uploaded")
                removed += 1
            else:
                cmd = ["aws", "s3", "rm", f"s3://{r2_bucket}/{key}",
                       "--endpoint-url", r2_endpoint, "--only-show-errors"]
                try:
                    res = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
                    if res.returncode == 0:
                        print(f"    ✅ Deleted {short_name} ({size_mb:.0f} MB) — split complete, no longer needed")
                        removed += 1
                except Exception:
                    pass
    ...
    if removed == 0:
        ...
        print(f"    Cleaned up {removed} legacy files")
...
def split(
    ...
    print(f"\n{'='*65}")
    print(f"  CrashLens split.py v3.0  |  {datetime.now():%Y-%m-%d %H:%M}")
    print(f"  Input  : {src.name}")
    print(f"  State  : {state_prefix}")
    print(f"  Output : {output_dir}")
    print(f"  Flags  : format=parquet.gz  upload={upload_r2}  dry_run={dry_run}")
    print(f"{'='*65}\n")
    ...
    stats["state"] = split_state_level(df, output_dir, dry_run)

    if not county_only:
        stats["regions"]            = split_region_level(df, hierarchy, output_dir, dry_run)
        stats["mpos"]               = split_mpo_level(df, hierarchy, output_dir, dry_run)
        stats["planning_districts"] = split_planning_district_level(df, hierarchy, output_dir, dry_run)

    stats["counties"] = split_county_level(df, hierarchy, output_dir, dry_run)
    stats["cities"]   = split_city_level(df, output_dir, dry_run)
    ...
    if upload_r2 and not dry_run:
        ...
        if endpoint:
            upload_to_r2(output_dir, state_prefix, r2_bucket, endpoint, dry_run)
            cleanup_legacy_r2(state_prefix, r2_bucket, endpoint, dry_run)
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="CrashLens Universal Jurisdiction & Road Type Splitter v3.0",
...
Examples:
  python split.py --input delaware_statewide_normalized.csv --state delaware
  python split.py --input virginia_statewide_normalized.csv --state virginia --upload-r2 --r2-endpoint https://xxx.r2.cloudflarestorage.com
  python split.py --input statewide.csv --state delaware --dry-run
  python split.py --input statewide.csv --state delaware --county-only
        """,
    )
