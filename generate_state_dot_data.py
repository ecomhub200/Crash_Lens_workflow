...
def download_features(endpoint_url, max_record_count=1000, out_sr=4326,
    ...
    # Helper: parse JSON from ArcGIS response (handles HTML/empty responses)
    def _parse_json(resp, context=""):
        """Parse JSON from response, handling common ArcGIS server quirks."""
        ct = resp.headers.get("Content-Type", "")
        text = resp.text.strip()

        # Empty response
        if not text:
            raise ValueError(f"{context}: Empty response (HTTP {resp.status_code})")

        # HTML response (server returned error page instead of JSON)
        if text.startswith("<!") or text.startswith("<html") or "text/html" in ct:
            # Some ArcGIS servers need POST instead of GET, or return HTML for GET
            raise ValueError(
                f"{context}: Server returned HTML instead of JSON. "
                f"Content-Type: {ct}, first 200 chars: {text[:200]}"
            )

        return resp.json()

    # First: get total count (try GET, then POST if HTML returned)
    count_params = {
        "where": "1=1",
    ...
    try:
        ...
        data = _parse_json(resp, "Count query (GET)")
        total = data.get("count", 0)
    except (ValueError, requests.exceptions.JSONDecodeError) as e:
        print(f"\n    GET failed: {e}")
        print(f"    Trying POST...")
        resp = requests.post(query_url, data=count_params, timeout=timeout)
        resp.raise_for_status()
        data = _parse_json(resp, "Count query (POST)")
        total = data.get("count", 0)
    ...
    try:
        ...
        meta_data = _parse_json(meta_resp, "Metadata")
        fields = meta_data.get("fields", [])
    except (ValueError, requests.exceptions.JSONDecodeError):
        # Try POST for metadata too
        meta_resp = requests.post(endpoint_url, data={"f": "json"}, timeout=timeout)
        meta_resp.raise_for_status()
        meta_data = _parse_json(meta_resp, "Metadata (POST)")
        fields = meta_data.get("fields", [])

    # Detect whether to use GET or POST for pagination
    use_post = False
    try:
        test_params = {
            "where": "1=1", "outFields": "OBJECTID", "outSR": out_sr,
            "returnGeometry": "false", "resultOffset": 0,
            "resultRecordCount": 1, "f": "json",
        }
        test_resp = requests.get(query_url, params=test_params, timeout=timeout)
        _parse_json(test_resp, "Test query")
    except (ValueError, requests.exceptions.JSONDecodeError):
        use_post = True
        print(f"    Server requires POST for queries")
    ...
    while offset < total:
        ...
        for attempt in range(3):
            try:
                if use_post:
                    resp = requests.post(query_url, data=params, timeout=timeout)
                else:
                    resp = requests.get(query_url, params=params, timeout=timeout)
                resp.raise_for_status()
                data = _parse_json(resp, f"Batch {batch_num}")
                break
