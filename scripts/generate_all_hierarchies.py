...
def make_region_id(state_abbr, district):
    ...
    Uses the existing ID if available (e.g. 'de_1', 'va_1'), otherwise
    falls back to _name_to_slug() on the district name for consistency
    with R2 folder naming.
    """
    ...
    return _name_to_slug(name) if name else 'unknown'
...
def _name_to_slug(name):
    ...
    If these diverge, Strategy A (column) and Strategy B (hierarchy) produce
    different R2 folder names, causing duplicate/stale folders.
...
def make_mpo_key(mpo):
    """Create R2-compatible slug from MPO name.

    Uses _name_to_slug() so hierarchy keys match what split.py and
    create_r2_folders.py generate from the MPO name string.
    """
    name = mpo.get('name', '')
    slug = _name_to_slug(name)
    return slug or mpo.get('id', 'unknown').lower().replace('-', '_')
