...
It detects and fixes gaps that would cause downstream ranking/aggregation failures:
  ...
  6. SLUG COLLISIONS — Two tpr/region entries that would produce the same R2 folder
  7. KEY-NAME MISMATCH — Hierarchy key doesn't match slug of the entry's name
...
def _name_to_slug(name):
    ...
    CRITICAL: This MUST match the slug logic in:
      - split.py: name_to_r2_key()
      - create_r2_folders.py: _name_to_slug()
      - generate_all_hierarchies.py: _name_to_slug()
    If these diverge, different pipeline stages produce different R2 folder
    names for the same jurisdiction, causing duplicates and data loss.

    Examples:
        "Dover/Kent County MPO"                       -> "dover_kent_county_mpo"
        "WILMAPCO (Wilmington Area Planning Council)"  -> "wilmapco_wilmington_area_planning_council"
        "St. Mary's"                                   -> "st_marys"
    """
    slug = name.lower()
...
def check_slug_collisions(hierarchy):
    """Detect tpr/region entries that would produce the same R2 folder slug.

    This is the root cause of the Delaware MPO bug: two entries like
    "dkcmpo" and "dover_kent_county_mpo" both represent Dover/Kent County MPO.
    When split.py writes to the slug-based folder, the second group overwrites
    the first, causing data loss.

    Auto-fix: remove the entry with fewer (or empty) counties, keeping the
    entry with the best data.
    """
    issues = []

    for section_name, section_key in [("tprs", "MPO"), ("regions", "Region")]:
        section = hierarchy.get(section_name, {})
        if not section:
            continue
        ...
        for key, data in section.items():
            ...
            slug_to_keys[slug].append((key, name, county_count))

        for slug, entries in slug_to_keys.items():
            if len(entries) > 1:
                # Keep the entry with most counties assigned
                entries.sort(key=lambda x: -x[2])
                keep = entries[0]
                for dup in entries[1:]:
                    def make_fix(s=section_name, k=dup[0]):
                        def fix(h):
                            h.get(s, {}).pop(k, None)
                            return True
                        return fix
                    ...
                    issues.append(HierarchyIssue(
                        ...
                        f"{section_key} slug collision: '{dup[1]}' ({dup[0]}) and "
                        f"'{keep[1]}' ({keep[0]}) both → '{slug}'. "
                        f"Removing '{dup[0]}' ({dup[2]} counties) in favor of "
                        f"'{keep[0]}' ({keep[2]} counties).",
                        fix_fn=make_fix()
                    ))
...
def check_key_name_mismatch(hierarchy):
    """Detect when a hierarchy key doesn't match the slug of its name.

    This causes Strategy A (column-based split using name_to_r2_key) and
    Strategy B (hierarchy-based split using the key directly) to produce
    different R2 folder names for the same jurisdiction.

    Example: key="dkcmpo" but name="Dover/Kent County MPO" → slug would be
    "dover_kent_county_mpo". Strategy A writes to dover_kent_county_mpo/,
    Strategy B writes to dkcmpo/ — two folders for one MPO.
    """
    issues = []
    ...
    for section_name, section_key in [("tprs", "MPO"), ("regions", "Region")]:
        ...
        for key, data in section.items():
            ...
            if expected_slug and key != expected_slug:
                issues.append(HierarchyIssue(
                    "warning", "key_name_mismatch",
                    f"{section_key} key '{key}' doesn't match slug of name "
                    f"'{name}' → expected '{expected_slug}'"
                ))
...
def validate_state(state_dir_name, dry_run=False, verbose=True):
    ...
    all_issues.extend(check_key_name_mismatch(hierarchy))
