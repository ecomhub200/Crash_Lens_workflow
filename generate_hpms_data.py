...
def get_state_hpms_url(state_name):
    """Build per-state HPMS FeatureServer URL from state name.
    
    Pattern: https://geo.dot.gov/server/rest/services/Hosted/{Name}_2018_PR/FeatureServer/0
    Multi-word states: "New Hampshire" → "New_Hampshire_2018_PR"
    """
    url_name = state_name.replace(" ", "_")
    url = f"{HPMS_BASE_URL}/{url_name}_2018_PR/FeatureServer/0"
    return url
...
def process_state(state_info, cache_dir, s3, bucket,
    ...
    # Method 1: Per-state ArcGIS REST API at geo.dot.gov
    state_url = get_state_hpms_url(name)
    print(f"      Method: ArcGIS per-state endpoint")
    print(f"      URL: {state_url}")
    try:
        if verify_hpms_endpoint(state_url):
            features = download_hpms_arcgis(state_url)
            if features:
