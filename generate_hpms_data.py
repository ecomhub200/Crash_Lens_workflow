...
# BTS GeoData Hub — HPMS 2023 (latest available as of 2025)
HPMS_ARCGIS_URLS = [
    # Try these in order until one works
    "https://geo.dot.gov/server/rest/services/Hosted/HPMS2023_Full/FeatureServer/0",
    "https://services.arcgis.com/xOi1kZaI0eWDREZv/arcgis/rest/services/HPMS2023_Full/FeatureServer/0",
    "https://geo.dot.gov/server/rest/services/Hosted/Highway_Performance_Monitoring_System_Full/FeatureServer/0",
    "https://services1.arcgis.com/Hp6G80Pky0om6HgA/arcgis/rest/services/HPMS2020/FeatureServer/0",
]
...
def discover_hpms_service():
    """Find a working HPMS ArcGIS FeatureServer endpoint."""
    for url in HPMS_ARCGIS_URLS:
        try:
            r = requests.get(f"{url}?f=json", timeout=15)
            if r.status_code == 200:
                data = r.json()
                if "fields" in data or "name" in data:
                    print(f"  HPMS service found: {url}")
                    return url
        except Exception:
            continue
    return None
...
def download_hpms_arcgis(state_fips, service_url, max_pages=500):
    """Download HPMS data for one state via ArcGIS REST API (paginated)."""
    all_features = []
    offset = 0
    page_size = 2000

    # Build field list — use * if specific fields fail
    out_fields = ",".join(HPMS_FIELDS) + ",SHAPE"

    for page in range(max_pages):
        params = {
            "where": f"State_Code={int(state_fips)}",
            "outFields": "*",
            "returnGeometry": "true",
...
def features_to_dataframe(features):
    ...
    for feat in features:
        ...
        signing = str(attrs.get("Route_Signing", "") or "")
        route_num = str(attrs.get("Route_Number", "") or "")
        route_name = str(attrs.get("Route_Name", "") or "").strip()
        if not route_name and route_num:
            sign_map = {"1": "I", "2": "US", "3": "SR", "4": "CR"}
            prefix = sign_map.get(signing, "")
            route_name = f"{prefix} {route_num}".strip()

        # Extract AADT — try multiple field name patterns
        aadt = attrs.get("AADT") or attrs.get("aadt") or attrs.get("Aadt") or 0
        try:
        ...
        rows.append({
            ...
            "f_system": int(attrs.get("F_System", 0) or 0),
            "aadt": aadt,
            "speed_limit": int(attrs.get("Speed_Limit", 0) or 0),
            "through_lanes": int(attrs.get("Through_Lanes", 0) or 0),
            "median_type": int(attrs.get("Median_Type", 0) or 0),
            "median_width": round(float(attrs.get("Median_Width", 0) or 0), 1),
            "surface_type": int(attrs.get("Surface_Type", 0) or 0),
            "ownership": int(attrs.get("Ownership", 0) or 0),
            "access_control": int(attrs.get("Access_Control", 0) or 0),
            "urban_code": str(attrs.get("Urban_Code", "") or ""),
            "facility_type": int(attrs.get("Facility_Type", 0) or 0),
            "route_name": route_name[:80],
            "terrain_type": int(attrs.get("Terrain_Type", 0) or 0),
            "curve_class": str(attrs.get("Curve_Class", "") or attrs.get("Curve_Cls", "") or ""),
            "grade_class": str(attrs.get("Grade_Class", "") or attrs.get("Grade_Cls", "") or ""),
            "nhs": int(attrs.get("NHS", 0) or 0),
            "county_code": str(attrs.get("County_Code", "") or ""),
            "length_mi": round(float(attrs.get("Section_Length", 0) or 0), 3),
            # NEW high-value fields
            "lane_width": round(float(attrs.get("Lane_Width", 0) or 0), 1),
            "shoulder_width_r": round(float(attrs.get("Shoulder_Width_R", 0) or attrs.get("R_Shoulder_Width", 0) or 0), 1),
            "shoulder_type_r": int(attrs.get("Shoulder_Type_R", 0) or attrs.get("R_Shoulder_Type", 0) or 0),
            "aadt_combination": int(attrs.get("AADT_Combination", 0) or attrs.get("AADT_Comb", 0) or 0),
            "aadt_single_unit": int(attrs.get("AADT_Single_Unit", 0) or attrs.get("AADT_SU", 0) or 0),
            "pct_peak_combination": round(float(attrs.get("Pct_Peak_Combination", 0) or attrs.get("Pct_Pk_Comb", 0) or 0), 1),
            "signal_type": int(attrs.get("Signal_Type", 0) or 0),
            "num_signals": int(attrs.get("Num_Signals", 0) or attrs.get("At_Grade_Other", 0) or 0),
            "turn_lanes_r": int(attrs.get("Turn_Lanes_R", 0) or 0),
            "turn_lanes_l": int(attrs.get("Turn_Lanes_L", 0) or 0),
            "design_speed": int(attrs.get("Weighted_Design_Speed", 0) or attrs.get("Design_Speed", 0) or 0),
            "peak_parking": int(attrs.get("Peak_Parking", 0) or 0),
            "iri": round(float(attrs.get("IRI", 0) or 0), 1),
            "k_factor": round(float(attrs.get("K_Factor", 0) or 0), 1),
            "dir_factor": round(float(attrs.get("Dir_Factor", 0) or 0), 1),
            "future_aadt": int(attrs.get("Future_AADT", 0) or 0),
            "num_signalized_int": int(attrs.get("Num_Signalized_Int", 0) or attrs.get("Num_Signals", 0) or 0),
            "num_stop_int": int(attrs.get("Num_Stop_Int", 0) or attrs.get("Stop_Signs", 0) or 0),
            "psr": round(float(attrs.get("PSR", 0) or 0), 1),
            "rutting": round(float(attrs.get("Rutting", 0) or 0), 1),
            "faulting": round(float(attrs.get("Faulting", 0) or 0), 1),
            "year_last_construction": int(attrs.get("Year_Last_Construction", 0) or attrs.get("Yr_Last_Constr", 0) or 0),
            "widening_feasibility": int(attrs.get("Widening_Feasibility", 0) or attrs.get("Widen_Feasiblty", 0) or 0),
            "shoulder_width_l": round(float(attrs.get("Shoulder_Width_L", 0) or attrs.get("L_Shoulder_Width", 0) or 0), 1),
            "shoulder_type_l": int(attrs.get("Shoulder_Type_L", 0) or attrs.get("L_Shoulder_Type", 0) or 0),
            "median_width": round(float(attrs.get("Median_Width", 0) or 0), 1),
            "pct_green_time": round(float(attrs.get("Pct_Green_Time", 0) or 0), 1),
        })
...
def process_state(state_info, cache_dir, s3, bucket, service_url,
                  force=False, local_only=False):
    """Download HPMS data for one state."""
    ...
    # Method 1: ArcGIS REST API
    if service_url:
        print(f"      Method: ArcGIS REST API")
        try:
            features = download_hpms_arcgis(fips, service_url)
            if features:
                df = features_to_dataframe(features)
                print(f"      ArcGIS: {len(df):,} road segments")
        except Exception as e:
...
HPMS FIELDS DOWNLOADED (63 per road segment):
    ...
    # Discover HPMS service
    print("Discovering HPMS ArcGIS service...")
    service_url = discover_hpms_service()
    if not service_url:
        print("  No ArcGIS service found — will use FHWA shapefile fallback")

    print(f"\n{'=' * 60}")
    print(f"  CrashLens HPMS Data Downloader")
    print(f"  States: {len(states)} | R2: {'yes' if s3 else 'local'}")
    print(f"  Source: {'ArcGIS + shapefile fallback' if service_url else 'FHWA shapefile only'}")
    print(f"{'=' * 60}")
    ...
    for i, state in enumerate(states, 1):
        ...
        try:
            result = process_state(
                state, cache_dir, s3, bucket, service_url,
