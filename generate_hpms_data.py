...
def features_to_dataframe(features):
    ...
    """
    # ── Field audit: log which fields the API actually returned ──
    if features:
        sample_attrs = features[0].get("attributes", {})
        actual_fields = set(sample_attrs.keys())
        expected_critical = {
            "AADT", "F_SYSTEM", "SPEED_LIMIT", "THROUGH_LANES", "SURFACE_TYPE",
            "OWNERSHIP", "ACCESS_CONTROL", "FACILITY_TYPE", "MEDIAN_TYPE",
            "URBAN_CODE", "LANE_WIDTH", "SHOULDER_WIDTH_R", "SHOULDER_WIDTH_L",
            "WEIGHTED_DESIGN_SPEED", "DESIGN_SPEED", "CURVE_CLASS", "GRADE_CLASS",
            "NUM_SIGNALIZED_INT", "NUM_STOP_INT", "NUM_SIGNALS",
            "YEAR_LAST_CONSTRUCTION", "SHOULDER_TYPE_R", "SHOULDER_TYPE_L",
            "COUNTY_CODE", "PSR", "WIDENING_FEASIBILITY", "PCT_PEAK_COMBINATION",
        }
        # Also check Title_Case and lowercase variants
        actual_upper = {f.upper() for f in actual_fields}
        found = expected_critical & actual_upper
        missing = expected_critical - actual_upper
        # Check if missing fields exist under different names
        alt_found = {}
        for f in missing:
            for actual in actual_fields:
                if f.lower().replace("_","") in actual.lower().replace("_",""):
                    alt_found[f] = actual
                    break
        print(f"      Field audit: {len(found)}/{len(expected_critical)} expected fields found")
        if missing - set(alt_found.keys()):
            truly_missing = missing - set(alt_found.keys())
            print(f"      ⚠️  Missing fields ({len(truly_missing)}): {sorted(truly_missing)[:10]}")
        if alt_found:
            print(f"      📋 Alternative names found: {alt_found}")
    rows = []
    for feat in features:
        ...
        if paths:
            ...
            if all_coords:
                ...
                # Compute length from geometry (haversine sum of coordinate pairs)
                import math
                total_m = 0.0
                ...
                geom_length_mi = round(total_m / 1609.34, 4)
        ...
        rows.append({
            ...
            "urban_code": str(g("URBAN_CODE", "Urban_Code", "URBAN_ID", "URBANIZED_AREA_CODE", "URBAN_AREA_CODE") or ""),
            ...
            "curve_class": str(g("CURVE_CLASS", "Curve_Class", "Curve_Cls", "CURVES", "Curve_Classification") or ""),
            "grade_class": str(g("GRADE_CLASS", "Grade_Class", "Grade_Cls", "GRADES", "Grade_Classification") or ""),
            "nhs": safe_int(g("NHS")),
            "county_code": str(g("COUNTY_CODE", "County_Code", "CNTY_CODE", "COUNTY_FIPS", "CTFIPS") or ""),
            "length_mi": safe_float(g("SECTION_LENGTH", "Section_Length", "MILES")) or geom_length_mi,
            "lane_width": safe_float(g("LANE_WIDTH", "Lane_Width", "LANE_WID")),
            "shoulder_width_r": safe_float(g("SHOULDER_WIDTH_R", "R_Shoulder_Width", "RSHL_WID")),
            "shoulder_type_r": safe_int(g("SHOULDER_TYPE_R", "R_Shoulder_Type", "RSHL_TYP")),
            "aadt_combination": safe_int(g("AADT_COMBINATION", "AADT_Combination", "AADT_COMB")),
            "aadt_single_unit": safe_int(g("AADT_SINGLE_UNIT", "AADT_Single_Unit", "AADT_SU")),
            "pct_peak_combination": safe_float(g("PCT_PEAK_COMBINATION", "Pct_Peak_Combination", "PCT_PK_COMB", "PCT_PK_C")),
            "signal_type": safe_int(g("SIGNAL_TYPE", "Signal_Type", "SIG_TYPE")),
            "num_signals": safe_int(g("NUM_SIGNALS", "Num_Signals", "At_Grade_Other", "NUM_SIGN", "AT_GRADE_OTHER")),
            "turn_lanes_r": safe_int(g("TURN_LANES_R", "Turn_Lanes_R", "TRN_LN_R")),
            "turn_lanes_l": safe_int(g("TURN_LANES_L", "Turn_Lanes_L", "TRN_LN_L")),
            "design_speed": safe_int(g("WEIGHTED_DESIGN_SPEED", "Design_Speed", "DSGN_SPD", "DESIGN_SPEED")),
            "peak_parking": safe_int(g("PEAK_PARKING", "Peak_Parking", "PK_PARKI")),
            ...
            "num_signalized_int": safe_int(g("NUM_SIGNALIZED_INT", "Num_Signalized_Int", "AT_GRADE_SIGNALIZED")),
            "num_stop_int": safe_int(g("NUM_STOP_INT", "Num_Stop_Int", "AT_GRADE_STOP")),
            "psr": safe_float(g("PSR", "PRESENT_SERVICEABILITY_RATING")),
            "rutting": safe_float(g("RUTTING", "Rutting")),
            "faulting": safe_float(g("FAULTING", "Faulting")),
            "year_last_construction": safe_int(g("YEAR_LAST_CONSTRUCTION", "Yr_Last_Constr", "YEAR_LAST_IMPROV", "YR_LAST_CONSTR")),
            "widening_feasibility": safe_int(g("WIDENING_FEASIBILITY", "Widen_Feasiblty", "WIDENING_OBSTACLE", "WIDEN_FEASIBLTY")),
            "shoulder_width_l": safe_float(g("SHOULDER_WIDTH_L", "L_Shoulder_Width", "LSHL_WID")),
            "shoulder_type_l": safe_int(g("SHOULDER_TYPE_L", "L_Shoulder_Type", "LSHL_TYP")),
