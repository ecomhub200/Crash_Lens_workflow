...
def download_features(endpoint_url, max_record_count=1000, out_sr=4326,
    ...
    """
    Download all features from an ArcGIS FeatureServer/MapServer via pagination.

    Handles common ArcGIS server quirks:
      - Empty responses (adds User-Agent header)
      - HTML instead of JSON (falls back to POST)
      - FeatureServer fails (falls back to MapServer)

    Args:
        endpoint_url: FeatureServer or MapServer layer URL
    ...
    """
    # Session with headers that prevent bot-blocking
    session = requests.Session()
    session.headers.update({
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) CrashLens/1.0",
        "Accept": "application/json",
        "Referer": endpoint_url.split("/arcgis/")[0] + "/",
    })

    def _try_json(resp, context=""):
        """Parse JSON, raising ValueError with debug info on failure."""
        ct = resp.headers.get("Content-Type", "")
        text = resp.text.strip()

        if resp.status_code != 200:
            raise ValueError(f"{context}: HTTP {resp.status_code}")
        if not text:
            raise ValueError(f"{context}: Empty response body (HTTP 200, CT={ct})")
        if text.startswith("<!") or text.startswith("<html") or "<html" in text[:200].lower():
            raise ValueError(f"{context}: HTML response. First 300: {text[:300]}")

        try:
            return resp.json()
        except Exception:
            raise ValueError(f"{context}: JSON parse failed. First 300: {text[:300]}")

    # Build list of URLs to try: original, then MapServer fallback
    urls_to_try = [endpoint_url]
    if "/FeatureServer/" in endpoint_url:
        urls_to_try.append(endpoint_url.replace("/FeatureServer/", "/MapServer/"))
    elif "/MapServer/" in endpoint_url:
        urls_to_try.append(endpoint_url.replace("/MapServer/", "/FeatureServer/"))

    # Try each URL with GET then POST
    query_url = None
    use_post = False
    total = 0
    fields = []
    ...
    for url in urls_to_try:
        qurl = f"{url}/query"
        svc_type = "FeatureServer" if "FeatureServer" in url else "MapServer"

        count_params = {"where": "1=1", "returnCountOnly": "true", "f": "json"}

        # Try GET
        for method, label in [("GET", "GET"), ("POST", "POST")]:
            try:
                if method == "GET":
                    resp = session.get(qurl, params=count_params, timeout=timeout)
                else:
                    resp = session.post(qurl, data=count_params, timeout=timeout)

                data = _try_json(resp, f"Count ({svc_type} {label})")
                ...
                if count > 0:
                    ...
                    print(f"  ✅ {svc_type} {label}: {total:,} features")

                    # Get field metadata
                    try:
                        if method == "GET":
                            mr = session.get(f"{url}?f=json", timeout=timeout)
                        else:
                            mr = session.post(url, data={"f": "json"}, timeout=timeout)
                        meta = _try_json(mr, "Metadata")
                    ...
                    print(f"    {svc_type} {label}: count=0 (may be wrong endpoint)")

            except ValueError as e:
                print(f"    ❌ {svc_type} {label}: {e}")
            except requests.exceptions.RequestException as e:
                print(f"    ❌ {svc_type} {label} network: {type(e).__name__}: {e}")

        if query_url:
            break

    if not query_url or total == 0:
        print(f"\n  ❌ All endpoints failed. Cannot download.")
        print(f"     Tried: {urls_to_try}")
        print(f"     Tip: Test manually: curl '{urls_to_try[0]}/query?where=1%3D1&returnCountOnly=true&f=json'")
        return [], []
    ...
    while offset < total:
        ...
        for attempt in range(3):
            try:
                if use_post:
                    resp = session.post(query_url, data=params, timeout=timeout)
                else:
                    resp = session.get(query_url, params=params, timeout=timeout)
                resp.raise_for_status()
                data = _try_json(resp, f"Batch {batch_num}")
                break
