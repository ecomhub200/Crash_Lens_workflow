...
Any state's normalize.py imports this module to derive missing columns:
  ...
  - MPO Name             (from hierarchy.json tprs, fallback to us_mpos.json centroid)
...
class GeoResolver:
    ...
    def _load_hierarchy(self, path: str):
        ...
        # Build county_fips → MPO name from tprs section
        # This is the PRIMARY source for MPO names — must match what split.py
        # and the frontend expect. The tprs section in hierarchy.json has
        # curated MPO names that are consistent with R2 folder naming.
        # Without this, fips_to_mpo stays empty and resolve_mpo() falls
        # through to us_mpos.json centroid matching, which returns BTS names
        # that can differ from hierarchy names (e.g. "Wilmington Area Planning
        # Council" instead of "WILMAPCO").
        tprs = data.get('tprs', {})
        for tpr_key, tpr_val in tprs.items():
            tpr_name = tpr_val.get('name') or tpr_val.get('shortName') or tpr_key
            for county_fips in tpr_val.get('counties', []):
                cf = str(county_fips).zfill(3)
                # Don't overwrite if region section already set an MPO for this county
                if cf not in self.fips_to_mpo:
                    self.fips_to_mpo[cf] = tpr_name
