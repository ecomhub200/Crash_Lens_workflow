...
Same output as generate_mapillary_data.py but downloads one county at a time:
  ...
  2. Downloads Mapillary features per county (adaptive tiling, 4 workers parallel)
  3. Saves per-county parquet to R2: {state}/{county_slug}/traffic-inventory.parquet.gz
  4. Consolidates all counties → statewide: {state}/cache/traffic-inventory.parquet.gz
  5. Skips counties already in R2 (restartable with --resume)
...
def process_county(county, abbr, state_name, r2_prefix, token, tile_size,
    ...
    try:
        ...
        elapsed = time.time() - t0
        safe_print(f"  [{index:3d}/{total}] {county_name} — FAILED ({elapsed:.0f}s): {str(e)[:60]}")
