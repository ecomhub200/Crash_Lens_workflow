#!/usr/bin/env python3
"""
osm_county_download.py — County-by-county OSM downloader for big states
========================================================================
...
REQUIRES: us_counties.json in states/geography/ or current directory
"""
...
def download_roads_by_county(state_name, state_abbr, state_fips):
    """Download road network county-by-county, return merged DataFrames."""
    import osmnx as ox

    counties = load_counties(state_fips)
    total = len(counties)
    print(f"      {total} counties found for {state_name}")
    ...
    for i, county in enumerate(counties, 1):
        county_name = county["NAMELSAD"]  # e.g. "Cuyahoga County"
        place = f"{county_name}, {state_name}, United States"

        print(f"      [{i:3d}/{total}] {county_name}...", end=" ", flush=True)
        t0 = time.time()
        ...
        try:
            ...
            print(f"{n_nodes:,} nodes, {n_edges:,} edges ({elapsed:.0f}s)")
    ...
    if failed:
        print(f"      ⚠️  {len(failed)} counties failed: {', '.join(failed[:5])}"
    ...
    print(f"      Merging {len(all_roads)} county DataFrames...", end=" ", flush=True)
    ...
    print(f"roads {before_roads:,}→{after_roads:,}, "
...
def download_pois_by_county(state_name, state_abbr, state_fips):
    """Download POIs county-by-county, return merged DataFrame."""
    import osmnx as ox

    counties = load_counties(state_fips)
    total = len(counties)
    print(f"      {total} counties — downloading POIs per county")

    all_pois = []
    category_counts = {cat: 0 for cat, _, _ in POI_TAGS}
    ...
    for i, county in enumerate(counties, 1):
        ...
        if i % 10 == 1 or i == total:
            print(f"      [{i:3d}/{total}] {county_name}...", flush=True)
        ...
        for category, osm_key, osm_values in POI_TAGS:
            try:
                ...
                pass  # Some counties may not have certain POI types
...
def main():
    parser = argparse.ArgumentParser(
        description="County-by-county OSM downloader for big states",
    )
    ...
    if do_roads:
        ...
        road_df, int_df = download_roads_by_county(state_name, abbr, state_fips)
    ...
    if do_pois:
        ...
        poi_df = download_pois_by_county(state_name, abbr, state_fips)
