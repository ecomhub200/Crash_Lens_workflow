    ...
    if do_roads:
        ...
        if workers <= 1:
            ...
            with ThreadPoolExecutor(max_workers=workers) as ex:
                futs = {ex.submit(_process_road, c, i): c["NAMELSAD"]
                        for i, c in enumerate(counties, 1)}
                for fut in as_completed(futs):
                    cn = futs[fut]
                    try:
                        name, rd, it = fut.result(timeout=COUNTY_TIMEOUT)
                        if rd is not None:
                            all_roads.append(rd)
                            if it is not None: all_ints.append(it)
                        else:
        ...
        # Retry failed (1 at a time, 60s cooldown)
        if failed:
