...
def test_ci_regressions(t):
    """Regression tests for bugs caught in CI."""
    print("\n── CI REGRESSION TESTS ──")

    def _authority_layer_signature():
        """Bug: apply_authority_layer() got unexpected kwarg 'state_abbr'"""
        import inspect
        import road_data_authority as rda
        sig = inspect.signature(rda.apply_authority_layer)
        params = list(sig.parameters.keys())
        assert "state_abbr" not in params, \
            f"apply_authority_layer still has state_abbr param: {params}"
        assert "df" in params, f"apply_authority_layer missing 'df' param: {params}"

    def _authority_layer_call_no_kwarg():
        """Bug: build_road_inventory.py passes state_abbr= to apply_authority_layer"""
        bri = _find_file("build_road_inventory.py")
        assert bri, "build_road_inventory.py not found"
        with open(bri) as f:
            content = f.read()
        assert "apply_authority_layer(roads, state_abbr" not in content, \
            "build_road_inventory.py still passes state_abbr to apply_authority_layer"

    def _split_check_no_get():
        """Bug: roads.get('Ownership', '') returns str not Series, .sum() fails"""
        bri = _find_file("build_road_inventory.py")
        assert bri, "build_road_inventory.py not found"
        with open(bri) as f:
            content = f.read()
        assert 'roads.get("Ownership", "")' not in content, \
            "build_road_inventory.py still uses roads.get('Ownership', '') pattern"

    t.test("apply_authority_layer signature (no state_abbr)", _authority_layer_signature)
    t.test("build_ri: no state_abbr kwarg in call", _authority_layer_call_no_kwarg)
    t.test("build_ri: no roads.get() for split check", _split_check_no_get)
