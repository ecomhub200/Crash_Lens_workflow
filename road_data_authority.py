...
HPMS_OWN_MAP = {
    ...
    4:  "1. State Hwy Agency",      # "Other State Agency" = DOT in DE, toll authority, etc.
...
# CRITICAL: HPMS codes are NOT 1:1 with VDOT numbering
HPMS_SURFACE_MAP = {
    1:  "1. Concrete",
    2:  "2. Blacktop, Asphalt, Bituminous",
    3:  "2. Blacktop, Asphalt, Bituminous",  # HPMS 3=Other Bituminous, NOT Brick
    4:  "2. Blacktop, Asphalt, Bituminous",  # HPMS 4=Bituminous Penetration
    5:  "2. Blacktop, Asphalt, Bituminous",  # HPMS 5=Mixed Bituminous
    6:  "3. Brick or Block",                  # HPMS 6=Brick
    7:  "4. Slag, Gravel, Stone",             # HPMS 7=Gravel
    8:  "4. Slag, Gravel, Stone",             # HPMS 8=Stone
    9:  "5. Dirt",                             # HPMS 9=Dirt
    10: "6. Other",                           # HPMS 10=Other
    11: "4. Slag, Gravel, Stone",             # HPMS 11=Slag
}
...
def populate_frontend_columns(df):
    """
    Map all resolved/enriched values to exact VDOT frontend column names.
    These columns are what crash_enricher.py and split.py expect.
    Must run AFTER apply_authority_layer.
    """
    ...
    if "resolved_functional_class" in df.columns:
        ...
        print(f"      Functional Class:  {fc_fill:>7,}/{n:,} ({fc_fill/n*100:.1f}%)")
        populated += 2
    ...
    if "resolved_ownership" in df.columns:
        ...
        print(f"      Ownership:         {own_fill:>7,}/{n:,} ({own_fill/n*100:.1f}%)")
        populated += 1

    # Facility Type
    if "resolved_facility_type" in df.columns:
        df["Facility Type"] = df["resolved_facility_type"]
        fac_fill = (df["Facility Type"] != "").sum()
        print(f"      Facility Type:     {fac_fill:>7,}/{n:,} ({fac_fill/n*100:.1f}%)")
        populated += 1

    # Roadway Surface Type
    if "resolved_surface_type_vdot" in df.columns:
        df["Roadway Surface Type"] = df["resolved_surface_type_vdot"]
        surf_fill = (df["Roadway Surface Type"] != "").sum()
        print(f"      Surface Type:      {surf_fill:>7,}/{n:,} ({surf_fill/n*100:.1f}%)")
        populated += 1

    # Traffic Control Type
    if "resolved_traffic_control" in df.columns:
        df["Traffic Control Type"] = df["resolved_traffic_control"]
        tc_fill = (df["Traffic Control Type"] != "1. No Traffic Control").sum()
        print(f"      Traffic Control:   {tc_fill:>7,}/{n:,} ({tc_fill/n*100:.1f}%)")
        populated += 1

    # Intersection Type
    if "resolved_intersection_type" in df.columns:
        df["Intersection Type"] = df["resolved_intersection_type"]
        it_fill = (df["Intersection Type"] != "1. Not at Intersection").sum()
        print(f"      Intersection Type: {it_fill:>7,}/{n:,} ({it_fill/n*100:.1f}%)")
        populated += 1
    ...
    # Geography columns
    geo_map = {
        "geo_dot_region": "DOT District",
        "geo_planning_district": "Planning District",
        "geo_mpo_name": "MPO Name",
        "geo_county_name": "Physical Juris Name",
        "geo_juris_code": "Juris Code",
    }
    for src, dst in geo_map.items():
    ...
    if "hpms_route_name" in df.columns:
        # HPMS fills, OSM name is fallback
        rte = df.get("hpms_route_name", pd.Series([""] * n)).astype(str).str.strip()
        osm_name = df.get("ref", pd.Series([""] * n)).astype(str).str.strip()
        rte = rte.where(rte != "", osm_name)
        rte = rte.where(rte != "0", "")
        df["RTE Name"] = rte
        populated += 1
    ...
    if "resolved_school_zone" in df.columns:
        df["School Zone"] = np.where(df["resolved_school_zone"] == "Yes",
                                      "1. Yes", "3. No")
        populated += 1
