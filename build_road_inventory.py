    ...
    for col in TIER_A_COLUMNS:
        ...
        if has_val.any():
            if col not in roads.columns:
                roads[col] = ""
            # Ensure type compatibility: pandas 3.0 + pyarrow is strict
            # about assigning numeric values to string columns
            assign_vals = sdot_vals[has_val]
            try:
                roads.loc[has_val, col] = assign_vals
            except (TypeError, ValueError):
                roads.loc[has_val, col] = [str(v) for v in assign_vals]
