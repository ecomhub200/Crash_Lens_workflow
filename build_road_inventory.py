...
def enrich_bridges(roads, df):
    enrich_nearest_asset(roads, df, "bridge", 500,
        attr_cols=[
            ...
            ("adt",             "adt"),
            ("width_m",         "width_ft"),   # renamed, converted below
            ("lanes",           "lanes"),
        ...
        label="Bridges")
    # Convert width from meters to feet
    col = "nearest_bridge_width_ft"
    if col in roads.columns:
        roads[col] = (pd.to_numeric(roads[col], errors="coerce").fillna(0) * 3.28084).round(1)
...
def main():
    ...
    # ── Post-enrichment data cleanup (state-agnostic) ──
    # Rail: cap trains_per_day at 500 (values above are likely highway AADT, not train counts)
    col = "nearest_rail_xing_trains_per_day"
    if col in roads.columns:
        vals = pd.to_numeric(roads[col], errors="coerce").fillna(0)
        roads[col] = np.where(vals > 500, 0, vals).astype(int)

    # Transit + Rail + Bridge: clean "Null"/"None" string values across all nearest_ columns
    for tc in [c for c in roads.columns if c.startswith("nearest_")]:
        if roads[tc].dtype == object or str(roads[tc].dtype) == "string":
            roads[tc] = roads[tc].astype(str).str.strip()
            roads.loc[roads[tc].isin(["Null", "null", "None", "none", "nan"]), tc] = ""
    ...
    # ── Column ordering: VDOT frontend columns FIRST ──
    
    # Group 1: VDOT Frontend Architecture (matches crash database columns)
    frontend = [
        "Functional Class", "SYSTEM", "Ownership", "Facility Type",
        "Roadway Surface Type", "Roadway Description", "Roadway Alignment",
        "Area Type", "DOT District", "Planning District", "MPO Name",
        "Physical Juris Name", "Juris Code",
        "RTE Name", "Max Speed Diff", "Through_Lanes", "AADT",
        "Traffic Control Type", "Intersection Type", "School Zone",
        "road_lon", "road_lat",
    ]

    # Group 2: Road segment identity
    road_id = ["road_source", "highway", "name", "ref", "oneway",
               "length_ft", "curvature", "osm_u_node", "osm_v_node",
               "u_lat", "u_lon", "v_lat", "v_lon", "mid_lat", "mid_lon"]

    # Group 3: Analysis — Confidence + Risk + Curve (clear names)
    analysis = sorted([c for c in roads.columns if c.startswith(("conf_","xval_","risk_","curve_"))])

    # Group 4: Geography (raw geo_ columns for split.py)
    geo_c = sorted([c for c in roads.columns if c.startswith("geo_")])

    # Group 5: Resolved source attribution
    resolved_c = sorted([c for c in roads.columns if c.startswith("resolved_")])

    # Group 6: Bridge / Rail / School / Transit (Near_ + nearest_ + count together)
    bridge_c = sorted([c for c in roads.columns
                       if ("bridge" in c.lower() or "Bridge" in c) and c not in frontend
                       and c not in ("bridge",)])  # exclude OSM raw "bridge" tag
    rail_c = sorted([c for c in roads.columns
                     if "rail_xing" in c or "RailXing" in c])
    school_c = sorted([c for c in roads.columns
                       if ("school" in c.lower() or "School" in c)
                       and c.startswith(("nearest_","Near_","school_"))
                       and c not in frontend])
    transit_c = sorted([c for c in roads.columns
                        if ("transit" in c.lower() or "Transit" in c)
                        and c.startswith(("nearest_","Near_","transit_"))])

    # Group 7: POI categories
    poi_c = sorted([c for c in roads.columns if c.startswith(("nearest_poi_","poi_","Near_Poi"))])

    # Group 8: HPMS raw (all 46 columns for deep analysis)
    hpms_c = sorted([c for c in roads.columns if c.startswith("hpms_")])

    # Group 9: OSM raw — only keep columns with meaningful data
    # Drop: tunnel(8/152K), surface(0), lit(0), sidewalk(0), cycleway(0), divider(0)
    # These are superseded by resolved_surface_type, resolved_has_lighting, etc.
    osm_raw = ["lanes", "maxspeed", "bridge"]

    # Group 10: Mapillary (all sign/infrastructure columns)
    map_c = sorted([c for c in roads.columns if c.startswith("map_")])

    # Group 11: Intersection raw
    ints_c = ["is_intersection", "intersection_degree"]

    # Assemble in order, deduplicating
    all_grouped = (frontend + road_id + analysis + geo_c + resolved_c +
                   bridge_c + rail_c + school_c + transit_c + poi_c +
                   hpms_c + osm_raw + ints_c + map_c)

    # Drop blank/superseded columns
    drop_cols = {"tunnel", "surface", "lit", "sidewalk", "cycleway", "divider", "length_m"}
    
    ordered = []
    seen = set()
    for c in all_grouped:
        if c in roads.columns and c not in seen and c not in drop_cols:
            ordered.append(c)
            seen.add(c)
    remaining = [c for c in roads.columns if c not in seen and c not in drop_cols]
    ...
    print(f"\n  Column groups:")
    fe_count = len([c for c in ordered if c in frontend])
    print(f"    VDOT Frontend:       {fe_count}")
    print(f"    Road identity:       {len([c for c in ordered if c in road_id])}")
    print(f"    Analysis:            {len(analysis)}")
    print(f"    Geography:           {len(geo_c)}")
    print(f"    Resolved sources:    {len(resolved_c)}")
    ...
    print(f"    HPMS raw:            {len(hpms_c)}")
    print(f"    Mapillary:           {len(map_c)}")
    ...
    map_yn = [c for c in map_c if c in roads.columns and roads[c].dtype == object
    ...
    map_cnt = [c for c in map_c if c.endswith("_count") or c.endswith("_100ft")]
    for c in map_cnt:
