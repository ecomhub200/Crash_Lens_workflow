...
def main():
    ...
    # Copy Census-derived area type to frontend column
    # (geo_area_type is dropped later as "duplicate geography" — this is the handoff)
    if "geo_area_type" in roads.columns:
        ...
        if valid_geo.any():
            ...
            print(f"    Area Type updated: Urban={n_u:,}, Suburban={n_s:,}, Rural={n_r:,}")
    ...
    if "highway" in roads.columns and "Functional Class" in roads.columns:
        ...
        if empty_fc.any():
            ...
            filled = resolved_fc != ""
            if filled.any():
                roads.loc[empty_fc & filled.values, "Functional Class"] = resolved_fc[filled].values
                ...
                own = roads["Ownership"].fillna("").astype(str)
                empty_own = empty_fc & (own == "")
                if empty_own.any():
                    new_own = resolved_fc.map(fc_to_own).fillna("")
                    own_filled = new_own != ""
                    roads.loc[empty_own & own_filled.values, "Ownership"] = new_own[own_filled].values
