...
def enrich_geography(roads, abbr, state_fips, cache_dir, s3, bucket):
    ...
    for local, r2_key in [
        ...
        (hierarchy_path, f"_national/snapshots/{abbr}/hierarchy.json"),
    ]:
        if not local.exists() and s3:
            try:
                s3.download_file(bucket, r2_key, str(local))
            except Exception:
                # Try alternate hierarchy paths
                if "hierarchy" in str(local):
                    for alt in [f"_national/hierarchy.json",
                                f"{STATES[abbr][1]}/hierarchy.json"]:
                        try:
                            s3.download_file(bucket, alt, str(local))
                            break
                        except Exception:
                            pass
